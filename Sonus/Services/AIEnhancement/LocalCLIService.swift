...
enum LocalCLITemplate: String, CaseIterable, Identifiable {
    ...
    var commandTemplate: String {
        switch self {
        case .pi:
            return "pi -ne -ns -p --no-tools --system-prompt \"$SONUS_SYSTEM_PROMPT\" \"$SONUS_USER_PROMPT\""
        case .claude:
            return "claude -p \"$SONUS_FULL_PROMPT\""
        case .codex:
            return "TMPFILE=$(mktemp) && codex exec --skip-git-repo-check --output-last-message \"$TMPFILE\" \"$SONUS_FULL_PROMPT\" > /dev/null 2>&1 && cat \"$TMPFILE\" && rm \"$TMPFILE\""
        case .copilot:
            return "copilot -p \"$SONUS_FULL_PROMPT\" -s --no-ask-user --available-tools=__none__ 2>/dev/null"
        }
    }
}
...
final class LocalCLIService {
    ...
    private static let shellPathQueue = DispatchQueue(label: "com.ronandonald.sonus.localcli.path")
    ...
    private func executeCommand(
        ...
        try await withCheckedThrowingContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                environment["SONUS_SYSTEM_PROMPT"] = systemPrompt
                environment["SONUS_USER_PROMPT"] = userPrompt
                environment["SONUS_FULL_PROMPT"] = fullPrompt
    ...
    private static func discoverPATHFromInteractiveLoginShell() -> String? {
        ...
        process.arguments = [
            "-ilc",
            "echo __SONUS_PATH_START__; print -r -- $PATH; echo __SONUS_PATH_END__"
        ]
        ...
        let startMarker = "__SONUS_PATH_START__"
        let endMarker = "__SONUS_PATH_END__"
